    ...
    async def _llm_analyze_completion(
        ...
        *,
        ...
        prompt = (
            ...
            f"   - IMPORTANT: Complete statements with factual claims are COMPLETE even if there's a trailing conjunction\n"
            f"   - Example: \"I figured out that 2+2=5 and\" → COMPLETE (substantive factual claim made)\n\n"
            f"3. SPEECH PATTERNS:\n"
            f"   - Trailing conjunctions (and, but, so) WITHOUT context → INCOMPLETE\n"
            f"   - BUT: If statement before conjunction is substantive → COMPLETE\n"
            f"   - Hesitation markers (um, uh, like) at END → may still be COMPLETE\n"
            f"   - Complete grammatical structure → COMPLETE\n"
            f"   - Sentence-ending punctuation (. ! ?) → ALWAYS COMPLETE\n\n"
        ...
        chat_ctx.add_message(
            ...
            content=(
                ...
                "You err on the side of COMPLETE when there's a substantive statement made, "
                "even with trailing words. Be conservative about marking things INCOMPLETE - "
                "only do so when truly mid-sentence with no complete idea expressed yet."
            )
        )
